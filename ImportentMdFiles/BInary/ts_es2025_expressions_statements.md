# TypeScript / ECMAScript 2025 — Complete Expressions & Statements Reference

> Intended for LLM consumption. Every expression and statement in the language is listed with a concise description, syntax, and a minimal example. Organized by category. Covers the ES2021 baseline plus everything added through ES2022, ES2023, ES2024, and ES2025.
...
| Dynamic import with attributes | `import("./data.json", { with: { type: "json" } })` | Second argument supplies import attributes (e.g., module type) to a dynamic import. (ES2025) |
...
| Name | Syntax | Description |
|---|---|---|
| Private field presence check | `#x in obj` | Tests whether an object has a given private field/method, without throwing if the class is unrelated. Only legal `in` form usable with a private name on the left. (ES2022) |
| Top-level `await` | `const data = await fetch(url);` (at module top level, no wrapping function) | `await` used directly in the body of a module (not inside any function). The module's evaluation pauses until the awaited value settles. (ES2022) |
| Numeric separator (recap) | `1_000_000` | Underscore used to visually group digits in numeric literals. (ES2021 — included here for completeness.) |
| RegExp `v` flag (`unicodeSets`) | `/[\p{Letter}--\p{ASCII}]/v` | Superset of the `u` flag adding set notation (`--` difference, `&&` intersection) and multi-character properties inside character classes. (ES2024) |
| Duplicate named capture groups | `/(?<year>\d{4})-|(?<year>\d{2})/` | The same named group (`year`) may appear in mutually exclusive alternatives of a single pattern. (ES2025) |
| RegExp inline pattern modifiers | `/^x(?i:HELLO)x$/`, `/^(?-i:CASE)sensitive$/i` | `(?flags:subpattern)` turns flags (`i`, `m`, `s`) on (after `?`) or off (after `-`) for just that subexpression, instead of the whole pattern. (ES2025) |
| `RegExp.escape()` | `RegExp.escape(str)` | Static method that escapes a string so it is safe to embed literally inside a `RegExp` pattern. (ES2025) |
...
| Import attributes | `import data from "./data.json" with { type: "json" };` | Attaches metadata to a static import declaration telling the host how to interpret the module (e.g., as JSON). Supersedes the earlier `assert { ... }` syntax. (ES2025) |
| Re-export with attributes | `export { default } from "./data.json" with { type: "json" };` | Import attributes also apply to re-export declarations that pull from another module. (ES2025) |
...
| Hashbang comment | `#!/usr/bin/env node` | A single `#!` line permitted only as the very first line of a script/module (before any other token), ignored by the engine. Formalizes what CLI tools like Node.js already stripped informally. (ES2023) |
| Top-level `await` (statement form) | `await import("./mod.js");` at the top of a module | See 1.9 — `await` used as a statement expression directly in module scope. (ES2022) |
...
| Decorator `[TS]` | `@sealed class Foo {}` | Applies a decorator function to a class, method, accessor, or property. Still **TC39 Stage 3** as of ES2025 — not yet finalized into the standard, but shipped natively by TypeScript 5+ and widely used via transpilers. |
| Accessor field (auto-accessor) `[TS]` | `accessor name = "";` | Declares a class field with an implicit, overridable getter/setter pair generated automatically. Introduced alongside the decorators proposal (Stage 3). |
...
## Appendix B — Notable ES2022–ES2025 Built-ins (not new syntax, but frequently confused with it)

These don't introduce new expression/statement *grammar* — they're ordinary method/property additions — but they're commonly asked about alongside language-level changes, so they're listed for completeness.

| Version | Additions |
|---|---|
| ES2022 | `Array.prototype.at()`, `String.prototype.at()`, `Object.hasOwn()`, `Error` constructor `cause` option (`new Error(msg, { cause })`), `structuredClone()` (HTML/host, widely bundled) |
| ES2023 | `Array.prototype.findLast()` / `findLastIndex()`, `Array.prototype.toReversed()` / `toSorted()` / `toSpliced()` / `with()` (non-mutating array copies), `Symbol` as `WeakMap`/`WeakSet` keys |
| ES2024 | `Object.groupBy()`, `Map.groupBy()`, `Promise.withResolvers()`, `ArrayBuffer.prototype.resize()` / `transfer()`, `SharedArrayBuffer.prototype.grow()`, `Atomics.waitAsync()`, `String.prototype.isWellFormed()` / `toWellFormed()` |
| ES2025 | `Iterator` global with helper methods (`.map()`, `.filter()`, `.take()`, `.drop()`, `.flatMap()`, `.reduce()`, `.toArray()`, etc. on any iterator), `Set.prototype.union()` / `intersection()` / `difference()` / `symmetricDifference()` / `isSubsetOf()` / `isSupersetOf()` / `isDisjointFrom()`, `Promise.try()`, `Float16Array`, redeclarable global `eval` vars |

---

## Appendix C — Looking Ahead (Stage 4, targeting ES2026 — *not yet* part of ES2025)

Flagged separately because they're easy to mistake for already-standard syntax:

| Name | Syntax | Status |
|---|---|---|
| `using` declaration | `using file = openFile(path);` | Explicit Resource Management — deterministically calls `[Symbol.dispose]()` at end of block scope. Reached Stage 4 in May 2025; targeted for ES2026, shipping in Chrome/Node/Deno already. |
| `await using` declaration | `await using conn = await openConnection();` | Async variant calling `[Symbol.asyncDispose]()`. Same proposal as above. |
| `Temporal` global | `Temporal.PlainDate.from("2026-03-11")` | Modern immutable date/time API intended to replace `Date`. Landing in ES2026, not part of ES2025. |
| `Array.fromAsync()` | `await Array.fromAsync(asyncIterable)` | Builds an array from an async iterable. Stage 4 as of May 2025, targeted ES2026. |
| `Error.isError()` | `Error.isError(x)` | Reliable cross-realm check for whether a value is an `Error`. Stage 4 as of May 2025, targeted ES2026. |
