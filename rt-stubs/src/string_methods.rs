...
pub unsafe extern "C" fn __bs_string_substring(str_tagged: u64, start_tagged: u64, end_tagged: u64) -> u64 {
    ...
    let mut start = if start_tagged == 0 { 0 } else {
        let val = f64::from_bits(start_tagged) as i64;
        std::cmp::max(0, std::cmp::min(val, len))
    };
    let mut end = if end_tagged == 0 { len } else {
        let val = f64::from_bits(end_tagged) as i64;
        std::cmp::max(0, std::cmp::min(val, len))
    };
