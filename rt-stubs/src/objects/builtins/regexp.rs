...
pub unsafe extern "C" fn regexp_test(env: u64, text_tagged: u64) -> u64 {
    ...
    let text_str = crate::get_c_string_from_tagged(text_tagged);
    let mut matched = false;
    
    if let Some(re) = build_regex(pattern_tagged, flags_tagged) {
        matched = re.is_match(&text_str);
    }
    gc::box_boolean(matched)
}

pub unsafe extern "C" fn regexp_exec(env: u64, text_tagged: u64) -> u64 {
    let closure_ptr = (env & 0x0000_FFFF_FFFF_FFFF) as *const u64;
    let obj_tagged = *(closure_ptr.add(1));
    let payload = obj_tagged & 0x0000_FFFF_FFFF_FFFF;
    ...
    if let Some(re) = build_regex(pattern_tagged, flags_tagged) {
        if let Some(caps) = re.captures(&text_str) {
            ...
            if let Some(m) = caps.get(0) {
                ...
                crate::objects::dynamic_props::set_dynamic_property(arr_payload as *mut u8, "index".to_string(), gc::box_number(index));
            }
...
pub unsafe extern "C" fn __bs_RegExp_new(pattern_tagged: u64, flags_tagged: u64) -> u64 {
    let obj = crate::__bs_alloc(&crate::REGEXP_VTABLE, 8);
    ...
    crate::objects::dynamic_props::set_dynamic_property(payload as *mut u8, "test".to_string(), create_method(obj, regexp_test as *const u8));
    crate::objects::dynamic_props::set_dynamic_property(payload as *mut u8, "exec".to_string(), create_method(obj, regexp_exec as *const u8));
    obj
}
